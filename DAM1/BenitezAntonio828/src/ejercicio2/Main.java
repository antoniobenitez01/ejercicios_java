...
public class Main {
	...
	public static void main(String[] args) 
	{
		...
		importarLlamadas(agenda, "llamadas.txt");
		for(Contacto i: agenda)
		{
			System.out.printf("= CONTACTO ="
					+ "\n%s\n\n", i.toString());
			if(!i.getRegistroLlamadas().isEmpty())
			{
				System.out.printf("- LLAMADAS -"+ "\n%s\n", i.llamadasToString());
			}
		}
	}
		...
		try 
		{
			...
			while(lector.hasNext())
			{
				...
				if(encontrado != null)
				{
					...
					encontrado.anadirLlamada(new Llamada(LocalDateTime.parse(datos[1]),tipo));
				}
			}
		} catch (FileNotFoundException e) 
		{
