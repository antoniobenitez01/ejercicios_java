...
public class RecetarioIO {
    ...
    {
    	Recetario recetarioLeido = null;
    	try
    	{
        	FileInputStream fichero = new FileInputStream(this.rutaArchivo);
        	ObjectInputStream objetosStream = new ObjectInputStream(fichero);
        	recetarioLeido = (Recetario) objetosStream.readObject();
        	objetosStream.close();
        }catch(IOException e)
    	{
    		System.out.printf("%s\n", e.getMessage());
    ...
    public void escribir(Recetario recetario) 
    {
    	try
    	{
        	FileOutputStream output = new FileOutputStream(this.rutaArchivo);
        	ObjectOutputStream outputStream = new ObjectOutputStream(output);
        	outputStream.writeObject(recetario);
        	outputStream.close();
        }catch(IOException e)
    	{
    		System.out.printf("%s\n", e.getMessage());
